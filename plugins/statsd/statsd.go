...
// statsd string into a usable metric struct
...
// If the line is valid, it will push it into the s.in channel to be collected
...
// bucketglob returns true if the bucket string matches the pattern string.
// Pattern is a glob, so foo.bar.*.* would match foo.bar.fee.fum
...
// caches the value in the s.counters or s.gauges map. It does not deal with the
// DeleteCounters or DeleteGauges or DeleteSets options, because those are dealt
...
func (s *Statsd) Stop() {
	...
	close(s.done)
	// TODO: flush the channel before closing?
	close(s.in)
}
